...
from transformers.models.dinov2.modeling_dinov2 import Dinov2LayerScale
...
class Dinov2OutProj(OutputProj):
    @classmethod
    def from_encoder(
        cls,
        encoder: HFVisionEncoder,
    ):
        model: Dinov2Model = encoder.model
        ...
        return Dinov2OutProj(
            mha_out_projs=nn.ModuleList(
                layer.attention.output for layer in model.encoder.layer
            ),
            layer_scale1s=nn.ModuleList(
                layer.layer_scale1 for layer in model.encoder.layer
            ),
    ...
    def __init__(
        ...
        self.layer_scale1s = layer_scale1s
    ...
    def project(self, unit_type2encodings: Mapping[str, torch.Tensor]):
        ...
        if "head" in unit_type2encodings:
            ...
            for layer_index in range(self.num_layers):
                ...
                layer_scale1: Dinov2LayerScale = self.layer_scale1s[layer_index]
                ...
                # apply layer scale1
                head_encoding = layer_scale1(head_encoding)
...
class Dinov2Tracer(ResidualTracer):
    def _enter(self):
        pass
    ...
    def __init__(
        ...
        super().__init__(
            ...
            raw=raw,
            out_proj=Dinov2OutProj.from_encoder(encoder=encoder),
            accumulate=accumulate,
            glob2fn=(
                {
                    "model.embeddings": self.emb_hook,
                    "model.encoder.layer.*.layer_scale2": self.mlp_hook,
